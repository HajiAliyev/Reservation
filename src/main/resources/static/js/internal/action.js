...
$('#reserveSubmitBtnId').click(function () {
    // var userId = 2;
    // var tableId = globClickedTableId;
    // var startTime = $('#startTimeInputId').val();
    // var endTime = $('#endTimeInputId')
    // var reservDate = $('#reservDateInputId').val();
    // var status = 'Y';

    $('#statusInputId').val('Y')
    $('#TableDbId').val(globClickedTableId);
    $('#userInputId').val(2);

    // $.ajax({
    //     url: 'reserveTableByUser',
    //     type: 'POST',
    //     data: 'type=' + type + '&description=' + description,
    //     dataType: 'html',
    //     success: function () {
    //         alert('Table is succesfully reserved, wait for submit of admin');
